...
class SignDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        X = np.zeros((MAX_LENGTH, len(LANDMARK_IDX), 3))
        X[:min(MAX_LENGTH, len(x))] = x[:MAX_LENGTH]
        X = X.reshape(MAX_LENGTH, -1)
        x_extracted = tsfel.time_series_features_extractor(self.cfg, X.T, verbose=0, n_jobs=None).values
        x_output = x_extracted.reshape(-1)
        # x = self.normalize(x)
        return x_output, y
